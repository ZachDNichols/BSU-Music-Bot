...
async function scrapeD2L() {
	...
	await page.waitForNavigation({waitUntil: 'networkidle0'});
	await page.waitForNavigation({waitUntil: 'networkidle0'});
	const body = await page.waitForSelector('div[class="d2l-page-main d2l-max-width d2l-min-width"] > div[class="d2l-page-main-padding"] > div[class="d2l-homepage"] > div[class="homepage-container"] > div[class="homepage-row"] > div' +
		...
		'div > div[class="d2l-datalist-container d2l-datalist-style1 d2l-datalist-block d2l-datalist-outdent"] > ul > li:nth-child(1) > div[class="d2l-datalist-item-content"] > div > div[class="d2l-htmlblock-untrusted"] > d2l-html-block');
	const innerText = (await body.getProperty('html')).jsonValue();
	await browser.close();
	return innerText;
}

module.exports = { scrapeD2L };